...
for record in vcf_in:
    ...
    if record.ref != ref_allele and ref_allele not in record.alts:
        print(
            "WARNING! It seems that the reference allele from the fasta file is not one of the variants in the vcf-file!"
        )
        not_present_var += 1

    if record.ref != ref_allele:
        print(
            f"Mismatch at {record.chrom}:{record.pos} - VCF: {record.ref}, REF: {ref_allele}"
        )
        if len(record.alts) > 1:
            print(
                "WARNING! There are multiallelic sites! The code should be modified accordingly!"
            )
